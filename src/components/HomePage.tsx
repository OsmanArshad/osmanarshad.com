          ...
          {DATA.education.map((education, index) => (
            <BlurFade key={education.school} delay={BLUR_FADE_DELAY * 8 + index * 0.05}>
              <div className="flex flex-col gap-3">
                ...
                {(education as any).courses && (education as any).courses.length > 0 && (
                  <div className="ml-11 md:ml-13 flex flex-col gap-2">
                    <span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Relevant Coursework</span>
                    <div className="flex flex-wrap gap-1.5">
                      {((education as any).courses as string[]).map((course: string) => (
                        <span
                          key={course}
                          className="text-xs border border-border rounded-md px-2 py-0.5 text-muted-foreground"
                        >
                          {course}
                        </span>
                      ))}
